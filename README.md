...
`UnsafeSort<T>` nor `PeriodicInt` classes have no mutable internal state, so it is recommended to use a single (non-static) instance
...
Data and test cases are in [BenchmarkResults](BenchmarkResults) directory; it contains raw XML reports generated by
BenchmarkDotNet as well as Excel workbook that loads and transforms the data.  Summarized report and analysis is
[here (PDF)](BenchmarkResults/Analysis.pdf).
