...
Sorter that stores the actual permutation in another array.

## Changes since v1

- Implemented Fisher-Yates shuffle; it is now used in benchmarks for more reliable validation of sorters.
- Support for arbitrary length arrays, i.e., not just lengths that are power of 2.  Specialized sorters for
  lengths of up to 16 are provided.
...
it runs an exhaustive validation of float networks for element counts of up to 32.  Larger sizes are not supported, as `2^N`
